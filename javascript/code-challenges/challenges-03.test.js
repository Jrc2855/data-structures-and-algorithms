...
let lessThanFour = numArr.filter((num)=> {
  return num > 4;
})
...
// charAt takes in an index and and returns the index with the value at the specified index
let filterStrArr = strArr.filter(str => str.charAt(1) === 'o');
let anotherFilteredArr = strArr.filter(str => str.length !== 3);

console.log(filterStrArr);
console.log(anotherFilteredArr);
...
const containsAnd = (arr) => {
  // Solution code here...
  const result = arr.filter(word => word.includes('and'));
  return result;
};
...
const oddValues = (arr) => {
  // Solution code here...
  const result = arr.filter(int => int % 2 !== 0);
  return result;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const result = arr.filter(arr => arr !== forbiddenValues);
  return result;
};
