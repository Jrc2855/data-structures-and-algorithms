...
const typeNum = (arr) => {
  // Solution code here...
  var justNumbers = function(value) {
    if(typeof(value) === 'number'){
      return value;
    }
  };
  let filteredArray = arr.filter(justNumbers);
...
function containsAnd(arr) {
  // Solution code here...
  let justAnd = function(string) {
    if(string.includes('and') === true){
      return string;
    }
  };
  let andArray = arr.filter(justAnd);
  return andArray;
}
...
const oddValues = (arr) => {
  // Solution code here...
  let oddsArray = arr.filter(number => number%2);
  return oddsArray;
};
// https://medium.com/front-end-weekly/understand-filter-in-javascript-6a3efe87173e
// learned more about this challenge through this link.
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let secondArray = function(value){
    for(let i = 0; i < arr.length; i++){
      if(value[i] !==forbiddenValues)
        return value[i];
    }
  };
  let filteredArr = arr.filter(secondArray);
